...
-- Entities this VM created with `vehicle:new`. They carry SetEntityOrphanMode(2), so they outlive
-- whoever spawned them: on a restart the old ones stayed in the world and the new run spawned
-- another copy on top (the same car, twice). Each resource takes its own with it when it stops.
local created = {}
...
--- Write a stream-synced statebag key on the vehicle: it replicates to every client that has the
--- entity streamed, which is how a fact about a car (its lock, its fuel, who owns it) reaches the
--- clients that have to act on it.
---
--- The mirror of `player_methods:setStreamSyncedMeta`, and of the client's own
--- `vehicle_methods:setStreamSyncedMeta`, so the same call means the same thing on both sides.
---
--- `false`, `0` and `nil` are written like anything else -- `nil` CLEARS the key, which is how a seed
--- from a previous session is taken back off a vehicle. It used to be called `streamSyncedMeta` and it
--- silently dropped every falsy value, so half the repo went around it with a raw `state:set` and the
--- other half had to guard each call; that is the bug this replaces, not a feature.
---@param key any
---@param value any
function vehicle_methods:setStreamSyncedMeta(key, value)
    Entity(self.entity).state:set(key, value, true)
end

--- Write several at once. One `state` lookup for the lot.
--- CANNOT clear keys: `{ key = nil }` stores nothing in a Lua constructor, so a nil entry never
--- reaches the loop. Clearing goes one key at a time through `setStreamSyncedMeta`.
---@param data table key/value pairs
function vehicle_methods:setStreamSyncedMetaMultiple(data)
...
-- Nobody else is going to clear them: orphan mode 2 means the engine keeps them forever, and the
-- entity handles only live in this table. A vehicle destroyed in the meantime is already out.
kec:on_resource_stop(function()
    for entity in pairs(created) do

local kec = ...
--[[
    mongodb — query builder + schemas/modelos (estilo Mongoose) sobre el
    bridge TypeScript del recurso `libs` (driver oficial de MongoDB).

    Este archivo es la fuente editable: se transpila a
    internal/modules/server/mongodb.lua y cada consumidor
    ...
    Uso rápido:
        local q = mongodb:collection("characters")
        local docs, err = q:where("account_id", id):sort({ _id = 1 }):find()

        mongodb:schema("Character", {
            collection = "characters",
            schema = { type = "object", properties = { ... }, required = { ... } },
            defaults = { position = { 0.0, 0.0, 72.0, 0.0 } }
        })
        local Character = mongodb:model("Character") -- desde cualquier recurso
...
function mongodb:collection(collectionName)
    ...
    --- Crea un índice y devuelve su nombre. `keys` es una lista ordenada de pares:
    ---   { { "account_id", 1 }, { "slotId", 1 } }
    --- Para TTL:
    ---   :createIndex({ { "expiresAt", 1 } }, { expireAfterSeconds = 0 })
    --- con docs que lleven expiresAt = mongodb:date(unix). Idempotente si
...
local function buildModel(name, definition)
    ...
    function model:collection()
        return mongodb:collection(self.collectionName)
    end
...
--- }
function mongodb:schema(name, definition)

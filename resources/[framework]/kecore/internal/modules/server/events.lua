...
function events:on_entity_created(handler)
    kec:onLocal("entityCreated", function(...)
...
function events:on_entity_creating(handler)
    kec:onLocal("entityCreating", function(...)
        if handler(...) == false then
            CancelEvent()
        end
    end)
...
function events:on_player_connected(handler)
    kec:onLocal("playerJoining", function(src)
...
function events:on_player_disconnect(handler)
    kec:onLocal("playerDropped", function(src, reason, resourceName, clientDropReason)
        local player = kec:player(src)
...
function events:on_player_bucket_changed(handler)
    kec:onLocal("kec:onPlayerBucketChanged", function(src, bucket, previous)
        local player = kec:player(src)
        if not player then return end
...
function events:on_player_connecting(handler)
    kec:onLocal("playerConnecting", function(src, _, setKickReason, deferrals)
        local player = kec:player(src)
        if player == nil then return end
...
--- Ejecuta `handler(player)` para cada jugador conectado cuando el recurso dueño se reinicia.
--- @param self any
--- @param handler function
function events:on_player_restart(handler)
    local invoking = owner

    kec:onLocal("onResourceStart", function(resourceName)
...
function events:on_weapon_damage(handler)
    kec:onLocal("weaponDamageEvent", function(sender, data)
...
function events:on_start_projectile(handler)
    kec:onLocal("startProjectileEvent", function(sender, data)
        local ret = handler(tonumber(sender), data)
        if ret == false then
            CancelEvent()
        end
...
function events:on_explosion(handler)
    kec:onLocal("explosionEvent", function(source, data)

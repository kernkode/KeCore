local enum = {
    ...
    -- El OTRO juego de "ropa" del ped: los accesorios van por su propio índice y con sus
    -- propias natives (SetPedPropIndex/ClearPedProp), no por el de componentes de arriba.
    -- Los dos índices empiezan en 0 y no tienen nada que ver: el 0 de aquí es un sombrero
    -- y el de ePedVarComp la cara, así que un número suelto no dice de cuál es.
    -- Del 3 al 5 no los usa el juego en peds freemode.
    ePedPropIdx = {
        HATS = 0,
        GLASSES = 1,
        EARS = 2,
    ...
    -- Huesos del ped: el ID (que es lo que comen GetPedBoneCoords y PointCamAtPedBone),
    -- no el índice de GetPedBoneIndex. Los nombres van tal cual los del juego para poder
    -- copiarlos de la documentación, y en orden de id.
    ePedBone = {
        SKEL_ROOT = 0,
    ...
    -- `kec.vehicle:classOf` y sus predicados (internal/modules/client/vehicle.lua).
    eVehicleClass = {
        COMPACT = 0,
        SEDAN = 1,
        SUV = 2,
